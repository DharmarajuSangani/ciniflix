  ...
  useEffect(() => {
    ...
    const fetchMovieDetails = async () => {
      try {
        ...
        const response = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=dd1b386e88b88aad32b851c505051f17`);
        const data = await response.json();
        setMovie(data);
        console.log(data.genres.map(genre => genre.name).join(", ")); // Log genre names to the console
      } catch (error) {
